import { MathStudentTabs } from "@/components/math-student-tabs";
import { getMathDemoStudent } from "@/lib/student-data";
import { formatDateLabel } from "@/lib/time";

export default function MathDemoLayout({
  children
}: Readonly<{
  ...
  return (
    <main className="dashboard-shell min-h-screen px-3 py-4 sm:px-4 sm:py-5">
      <div className="mx-auto flex min-h-[calc(100vh-2rem)] max-w-[430px] flex-col gap-4 rounded-[32px] border border-white/55 bg-[rgba(248,250,252,0.7)] p-3 shadow-[var(--shadow)] backdrop-blur-xl sm:p-4">
        <header className="rounded-[28px] border border-[var(--line)] bg-[var(--panel)] p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[11px] font-medium uppercase tracking-[0.28em] text-[var(--muted)]">
                Live Dashboard
              </p>
              <h1 className="mt-2 text-[32px] font-bold leading-tight text-[var(--ink)]">
          ...
          <div className="mt-4 rounded-2xl bg-slate-950 px-4 py-4 text-white">
            ...
            <p className="mt-3 text-[15px] leading-7 text-slate-200">{student.target_band}</p>
          </div>
        </header>
